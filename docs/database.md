...
- `auth`: reservado às tabelas do Better Auth, que serão adicionadas somente após o spike de compatibilidade.
...
`db:generate` altera arquivos versionados, então o SQL e o snapshot devem ser revisados. `db:migrate` é uma ação explícita de release/operação; nenhuma aplicação executa migration durante o boot. `drizzle-kit push` não faz parte do fluxo aprovado.
